  ...
  Using_Futures() {
    ...
    client.todos()
      .then((List existingTodos) {
        Future.wait(existingTodos.mappedBy((x) => client.delete('todos/${x['id']}')).toList())
          .then((_) {
              int i=0;
              Future.wait(todos.mappedBy((text) => client.todos({'content':text, 'order':i++})).toList())
